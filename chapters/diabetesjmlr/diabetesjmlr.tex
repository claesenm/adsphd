...
\subsection{Performance Curves} \label{resvmroc}
The RESVM model based on \textsc{atc $|$ provs} vectorization had the best overall performance. Figure~\ref{fig:diabetes-performance-curves} shows bounds on the ROC and PR curves for this model and includes curves for the CWSVM model based on age and gender and a random model for reference. These bounds were computed using the technique described in Chapter~\ref{ch:evaluation}. The true curve is unknown because we do not dispose of negative labels.
...
\caption{Performance curves for RESVM classifier based on \textsc{atc $|$ provs} vectorization, CWSVM based on age and gender and a random model. The lower and upper bounds are estimated using $\hat{\beta}_{lo}=5\%$ and $\hat{\beta}_{up}=10\%$, respectively.}
\label{fig:diabetes-performance-curves}
\end{figure}

It should be noted that precision depends on class balance, and therefore the PR curve shown in Figure~\ref{fig:pr-diabetes} is not representative for screening an overall population, since the overall population has a higher fraction of negatives than our custom data set (i.e. precision would be lower in practice). In contrast, the bounds in ROC space are representative because ROC curves are insensitive to changes in class distribution \citep{Fawcett:2006:IRA:1159473.1159475}. What matters in the PR curves is the shape of the curve for RESVM, which peaks at low recall, while the curves for CWSVM and a random model do not. The CWSVM approach is similar to the existing guidelines outlined in Table~\ref{intro:diabetes-liga-risk}, which essentially identify persons at high risk through age and a handful additional risk factors. The performance curves indicates that the RESVM model is good for case finding (which requires high precision) and far outperforms screening based on only age and gender.
...
\begin{equation}
\bar{\mathbf{w}} = \sum_{i=1}^{n_{\rm models}} \mathbf{w}_i / n_{\rm models}. \label{eq:featureweights}
\end{equation}
...
%Via this approach, the risk associated to use of cardiovascular medication (ATC main category \textsc{c}) far outweights all other ATC main categories. This is not surprising, as diabetes is known to be strongly related to cardiovascular problems \citep{doi:10.1001/jama.1979.03290450033020, grundy1999diabetes, hu2002elevated}. 

Table~\ref{diab:features} depicts the level 3 ATC codes that contribute most to diabetes risk based on the metric outlined above. As expected, cardiovascular medication (ATC \texttt{C*}) is identified as a clear risk indicator \citep{doi:10.1001/jama.1979.03290450033020, grundy1999diabetes, hu2002elevated}. However, a number of less obvious indicators are also revealed, specifically antipsychotics (\texttt{N05A}), antidepressants (\texttt{N06A}) and antigout preparations (\texttt{M04A}), for which associations with T2D are known \citep{clark2004consensus, anderson2001prevalence, mokdad2003prevalence}. The relative importance of features and associated medical implications will be discussed in detail in a forthcoming medical paper.


\newcommand{\plotweight}[1]{
...
%\addplot[gray, thick, dashed] coordinates {(0.0, -1) (0.0, 1)};
%\addplot[gray, thick, dashed] coordinates {(1.0, -1) (1.0, 1)};
%\addplot[gray, thick, dashed] coordinates {(0.71, -1) (0.71, 1)};
%\addplot[gray, thick, dashed] coordinates {(0.83, -1) (0.83, 1)};
%\addplot[black, dotted] coordinates {(0.25, -1) (0.25, 1)};
%\addplot[black, dotted] coordinates {(0.50, -1) (0.50, 1)};
%\addplot[black, dotted] coordinates {(0.75, -1) (0.75, 1)};
%\addplot[color=white] coordinates {(0, -1) (0, 1)};
\draw [color=cyan!80!black, opacity=0.8] (axis cs: 0, -0.7) rectangle (axis cs: #1, 0.6);
\fill [color=cyan!80!black, opacity=0.4, fill] (axis cs: 0, -0.7) rectangle (axis cs: #1, 0.6);
    \end{axis}
\end{tikzpicture}
}
...
\begin{tabular}{cl}
\toprule
feature & normalized weight as per Equation~\eqref{eq:featureweights} \\
\midrule
\texttt{C09B} & \plotweight{1.000} 788.3 \\
\texttt{C09D} & \plotweight{0.829} 653.8 \\
\texttt{C10A} & \plotweight{0.638} 503.0 \\
\texttt{N05A} & \plotweight{0.551} 434.3 \\
\texttt{C03E} & \plotweight{0.524} 412.7 \\
\texttt{C03C} & \plotweight{0.513} 404.2 \\
\texttt{N06A} & \plotweight{0.511} 403.1 \\
\texttt{M04A} & \plotweight{0.504} 397.0 \\
\texttt{A02B} & \plotweight{0.480} 378.5 \\
\texttt{C09A} & \plotweight{0.432} 340.1 \\
\bottomrule
\end{tabular}
\caption{Level 3 ATC codes with highest normalized feature weight.} \label{diab:features}
\end{table}
