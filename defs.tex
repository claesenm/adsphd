% Some useful packages
\usepackage{amssymb,amsthm}
\usepackage{amsmath}

% useful: TODO notes
\usepackage{todonotes}
%fix placement of todo notes (see todonotes manual)
\setlength{\marginparwidth}{(\stockwidth-\columnwidth)/2-.35cm}

% Some nice colors
\newcommand{\red}[1]{\textcolor{red}{#1}}
\newcommand{\green}[1]{\textcolor{green}{#1}}
\newcommand{\blue}[1]{\textcolor{blue}{#1}}
\newcommand{\cyan}[1]{\textcolor{cyan}{#1}}
\newcommand{\magenta}[1]{\textcolor{magenta}{#1}}
\newcommand{\yellow}[1]{\textcolor{yellow}{#1}}
\newcommand{\orange}[1]{\textcolor{orange}{#1}}
\newcommand{\violet}[1]{\textcolor{violet}{#1}}
\newcommand{\purple}[1]{\textcolor{purple}{#1}}
\newcommand{\brown}[1]{\textcolor{brown}{#1}}
\newcommand{\gray}[1]{\textcolor{gray}{#1}}
\newcommand{\darkgray}[1]{\textcolor{darkgray}{#1}}
\newcommand{\lightgray}[1]{\textcolor{lightgray}{#1}}
\newcommand{\bred}[1]{\textbf{\textcolor{red}{#1}}}

\definecolor{MyGray}{rgb}{0.91,0.91,0.93}
\makeatletter\newenvironment{graybox}{%
   \begin{lrbox}{\@tempboxa}\begin{minipage}{\columnwidth}}{\end{minipage}\end{lrbox}%
   \colorbox{MyGray}{\usebox{\@tempboxa}}
}\makeatother

\newcommand{\chapterfrontpage}[3]{
	\begin{graybox}
	This chapter has been previously published as: \\
%	\ \newline
	#1
	% http://gking.harvard.edu/files/bibentry2.pdf
%	\bibentry{#1}
%	\fullcite{#1}
	\end{graybox}
	\vfill
	\vspace{-5mm}
	\paragraph{Contributions}
	#2
	\section*{Abstract}
	#3
}

\newcommand{\chapterfrontpagesubmitted}[3]{
	\begin{graybox}
	This chapter has been submitted as: \\
%	\ \newline
	#1
	% http://gking.harvard.edu/files/bibentry2.pdf
%	\bibentry{#1}
%	\fullcite{#1}
	\end{graybox}
	\vfill
	\paragraph{Contributions}
	#2
	\section*{Abstract}
	#3
}

\newcommand{\chapterfrontpagetosubmit}[4]{
	\begin{graybox}
	This chapter will be submitted to \emph{#4} as:\\
%	\ \newline
	#1
	% http://gking.harvard.edu/files/bibentry2.pdf
%	\bibentry{#1}
%	\fullcite{#1}
	\end{graybox}
	\vfill
	\paragraph{Contributions}
	#2
	\section*{Abstract}
	#3
}
